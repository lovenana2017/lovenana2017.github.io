            ...
            //$(function() {
				...
				function disperse(){
					...
					var spaces_h 	= $(window).height()*0.7/(per_col + 1);
					...
					$thumbs.each(function(i){
						...
						var top		= spaces_h*(Math.ceil((i+1)/per_line)) - $thumb.height()*0.7/2;
						//lets give a random degree to each thumb
						var r 		= Math.floor(Math.random()*41)-20;
						/*
						now we animate the thumb to its final positions;
						we also fade in its image, animate it to 177.5x177.5,
					...
					if(mode	== 'grid'){
						...
						$thumbs.each(function(i){
							...
							$image.stop().animate({
								...
								var f_w	= per_line * 187.5;//187.5
								var f_h	= per_col * 140.625;//140.625
								var f_l = $(window).width()/2 - f_w/2;
								var f_t = $(window).height()*0.7/2 - f_h/2;
								/*
								...
								if(ie)
									var param = {
										'left'	: f_l + (i%per_line)*187.5 + 'px',
										'top'	: f_t + Math.floor(i/per_line)*140.625 + 'px'
									};
								else
									var param = {
										'rotate': '0deg',
										'left'	: f_l + (i%per_line)*187.5 + 'px',//(i%per_line)*187.5 + 'px',
										'top'	: f_t + Math.floor(i/per_line)*140.625 + 'px'
									};
								$thumb.css({
				...
				$(window).smartresize(function(){
					...
					TweenMax.to(".boat", 1, {
					  x: function(index, target) {
					    return (index-newW) *0.95; // adjust  travel length according to width
					  }
					});
				});
